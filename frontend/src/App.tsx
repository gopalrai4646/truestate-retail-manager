...
import { fetchTransactions, uploadDataset } from './services/api';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const initData = async () => {
      ...
      // Wait a moment for XLSX to load from CDN if needed
      if (!(window as any).XLSX) {
        await new Promise(resolve => setTimeout(resolve, 500));
      }

      try {
        // Attempt to auto-load only if present in root (for dev/demo purposes)
        const response = await fetch('/dataset.csv');
        if (response.ok) {
          const buffer = await response.arrayBuffer();
          const mappedData = parseExcelData(buffer);
          await uploadDataset(mappedData);
        }
      } catch (error) {
        // Silent fail on auto-load, user will see empty state with upload button
        console.log("No default dataset found, waiting for user upload.");
      } finally {
        loadTransactions();
      }
    };
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (evt) => {
      ...
      try {
        const mappedData = parseExcelData(data as ArrayBuffer);
        await uploadDataset(mappedData);
        setPagination(prev => ({ ...prev, currentPage: 1 }));
        loadTransactions();
  ...
  const stats = useMemo(() => {
    ...
    return { units: totalQty, amount: totalAmt, discount: totalDisc };
  }, [transactions]);
